...
export function alignmentHandling(){
    modifyPage();
}
    ...
    await chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
        chrome.tabs.sendMessage(tabs[0].id, { action: 'get_current_url' }, async function(response) {
            if (response && response.url) {
                ...
                const align=localInfo.policyAlignment;
                definePreferences();
                fillLists(align,document.getElementById("summaryContentDisplayContainerBody"));

                document.getElementById('alignmentConflictCounterSpan').innerText=align.length;
            }
        });
    });
